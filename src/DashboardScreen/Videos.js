...
const Videos = () => {
    ...
    const handleFile = (selectedFile) => {
        // Updated size limit to 2 GB
        if (selectedFile && selectedFile.size <= 2147483648) { // Limit of 2 GB
            setVideoFile(selectedFile);
            setError(''); // Clear any existing error
        } else {
            setError('Veuillez sélectionner un fichier vidéo de moins de 2 Go.'); // Updated error message
        }
    };
    ...
    const handleUpload = async (event) => {
        ...
        try {
            await axios.post('https://talelgym.tn/apipsy/upload.php', formData, {
